...
class TableDetectionModel:
    ...
    def batch_detect(self, input_batch: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        # Define the per-batch inference code in `__call__`.
        batch = [image for image in input_batch["transformed"]]
        paths = [path for path in input_batch["path"]]
        return self.detect(batch, paths, 0.3)

    def detect(self, image,  paths,  confidence):
...
def run_job():
    ...
    minio_access_key = os.getenv("AWS_ACCESS_KEY_ID", "Altilia.2021") 
    minio_secret_key = os.getenv("AWS_SECRET_ACCESS_KEY", "Altilia.2021") 
    tenant = os.getenv("TENANT", "altilia")
    user = os.getenv("USERNAME", "andrea.dodero")
    project = os.getenv("PROJECT", "1")
    kb = os.getenv("KNOWLEDGE_BASE", "1")
    partition = os.getenv("PARTITION", "partizione")
    orchestratorRef = os.getenv("orchestratorRef", "id-conductor-wf")

    processing_folder = f"s3://{tenant}/{user}/{project}/{kb}/{partition}"

    fs3 = pyarrow.fs.S3FileSystem(access_key=minio_access_key,
                                secret_key=minio_secret_key,
                                endpoint_override=minio_endpoint,
                                scheme='http')

    ds = ray.data.read_binary_files(processing_folder,
                                    include_paths=True,
    ...
    results.flat_map(lambda row:  row['predictions']).write_json(f"{processing_folder}/tables", filesystem=fs3)

    notify_conductor(orchestratorRef, tenant, user)
